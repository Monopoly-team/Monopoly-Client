...
{
    if (cell.ownerId == NO_OWNER_ID || cell.rent.isEmpty()) {
        return 0;
    }
    ...
    if (cell.type == CellType::ExtraBusiness) {
        ...
        const int rentIndex = clampValue(businessCount - 1, 0, cell.rent.size() - 1);
    ...
    const int buildingLevel = clampValue(cell.buildingLevel, 0, cell.rent.size() - 1);
    int rent = cell.rent[buildingLevel];
...
bool GameRules::ownsFullGroup(const GameState& state, int ownerId, PropertyGroup group)
{
    if (ownerId == NO_OWNER_ID || group == PropertyGroup::None) {
        return false;
    }
