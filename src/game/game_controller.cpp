...
void GameController::addPlayer(int playerId, const QString& nickname)
{
    ...
    if (!session_.addPlayer(playerId, nickname)) {
...
bool GameController::handlePlayerAction(int playerId, const QJsonObject& action)
{
    GameState& state = session_.state();
    ...
    const QString actionType = actionTypeFromJson(action);
    ...
    if (actionType == "roll_dice" || actionType == "roll") {
        isHandled = handleRollDiceAction(*player, action);
        ...
        isHandled = handleBuildBusinessAction(*player, action);
...
QString GameController::actionTypeFromJson(const QJsonObject& action) const
{
    QString actionType = action["action"].toString().trimmed();

    if (actionType.isEmpty()) {
        actionType = action["type"].toString().trimmed();
    }

    return actionType.toLower();
}

bool GameController::handleRollDiceAction(Player& player, const QJsonObject& action)
{
    ...
    int dice1 = action["dice1"].toInt(0);
    int dice2 = action["dice2"].toInt(0);
    int steps = action["steps"].toInt(0);
    ...
    if (steps <= 0) {
        ...
        appendEvent(
            QStringLiteral("%1 бросил кубики: %2 и %3. Ход на %4 клеток.")
        ...
        appendEvent(
            QStringLiteral("%1 ходит на %2 клеток.")
                .arg(playerName(player))
                .arg(steps));
    }
...
bool GameController::handleBuildBusinessAction(Player& player, const QJsonObject& action)
{
    const int cellId = action.contains("cellId")
    ? action["cellId"].toInt(-1)
    : player.position;
...
void GameController::handleStartCell(Player& player)
{
    appendEvent(QStringLiteral("%1 остановился на Start.").arg(playerName(player)));
}

void GameController::handleGoToJailCell(Player& player)
{
...
void GameController::collectFromOtherPlayers(Player& player, int amountPerPlayer, const QString& reason)
{
    ...
    for (Player& otherPlayer : session_.state().players()) {
        if (otherPlayer.id == player.id || otherPlayer.isBankrupt) {
            continue;
        }
